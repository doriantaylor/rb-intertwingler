source 'https://rubygems.org'

# uggggggggggghhhhhhhh
# gem 'uri-urn', '0.0.5',
#   git: 'https://gitlab.com/doriantaylor/uri-urn.git', branch: 'master'

# do this until upstream merges
gem 'mimemagic',
  git: 'https://github.com/doriantaylor/rb-mimemagic.git',
  branch: 'aliases-etc'

gem 'tty-prompt', '0.23.1',
  git: 'https://github.com/doriantaylor/rb-tty-prompt.git',
  branch: 'add-prompt-to-collector'

gem 'tty-reader', '0.9.0',
  git: 'https://github.com/doriantaylor/rb-tty-reader.git',
  branch: 'control-w-please'

gem 'store-digest', '0.1.4',
  git: 'https://github.com/doriantaylor/rb-store-digest.git'

gem 'store-digest-http', '0.1.1',
  git: 'https://github.com/doriantaylor/rb-store-digest-http.git'

# XXX why is this in here?? oh, version, duh
# gem 'md-noko', '0.1.0', git: 'https://github.com/doriantaylor/rb-md-noko.git'

# Specify your gem's dependencies in intertwingler.gemspec
gemspec
